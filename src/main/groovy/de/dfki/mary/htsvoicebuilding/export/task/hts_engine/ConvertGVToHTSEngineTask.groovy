 ...
 *
 */
public class ConvertGVToHTSEngineTask extends DefaultTask {
    /** The worker */
     ...
     */
    @Inject
    public ConvertGVToHTSEngineTask(WorkerExecutor workerExecutor) {
        super();
    ...
    public void generate() {
        ...
        for (def stream: project.vb_configuration.models.cmp.streams) {
            ...
            for (File cur_file: script_files.getFiles()) {
                if (cur_file.getName().startsWith("cv_hts_engine_gv_${stream.kind}.")) {
            ...
            for (File cur_file: output_tree_files.getFiles()) {
                if (cur_file.getName().startsWith("gv_${stream.kind}.")) {
            ...
            workerExecutor.submit(ConvertGVToHTSEngineWorker.class,
 ...
 *
 */
class ConvertGVToHTSEngineWorker implements Runnable {
     ...
     */
    @Inject
    public ConvertGVToHTSEngineWorker(File template_file, File list_file, int stream_id,
    ...
    @Override
    public void run() {
        def tmp_dir = new File(output_model_file.getParent().toString() + "/gv")
        tmp_dir.mkdirs()
