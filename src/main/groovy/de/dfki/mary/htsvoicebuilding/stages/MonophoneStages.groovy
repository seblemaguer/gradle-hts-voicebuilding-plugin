...
class MonophoneStages {
    ...
    public static void addTasks(Project project)
    {
        project.task('initialiseMonophoneModels', type: InitPhoneModelsTask)  {
            ...
            // Output cmp files
            cmp_hinit_dir = project.file("$project.cmp_model_dir/HInit/")
            cmp_hrest_dir = project.file("$project.cmp_model_dir/HRest/")

            // Output dur files
            dur_hrest_dir = project.file("$project.dur_model_dir/HRest/")
        }
        ...
        project.task('generateMonophoneMMF', type:GenerateMonophoneModelTask) {
            ...
            // Scripts part
            script_template_cmp_file = project.file("${project.template_dir}/lvf.hed")
            script_cmp_file = project.file("${project.hhed_script_dir}/lvf.cmp.hed")
            script_dur_file = project.file("${project.hhed_script_dir}/lvf.dur.hed")
            ...
            // Model filename
            cmp_mmf_file = project.file("${project.cmp_model_dir}/monophone/init/monophone.mmf")
            dur_mmf_file = project.file("${project.dur_model_dir}/monophone/init/monophone.mmf")
        ...
        project.task('trainMonophoneMMF', type: TrainModelsTask) {
            ...
            trained_cmp_file = project.file("${project.cmp_model_dir}/monophone/trained/monophone.mmf")
            trained_dur_file = project.file("${project.dur_model_dir}/monophone/trained/monophone.mmf")
        }
    }
}
